...
import 'package:connettion/ui/subpagedanhsachlop.dart';
import 'package:connettion/ui/subpagediemdanh.dart';
import 'package:connettion/ui/subpagedshocphan.dart';
import 'package:connettion/ui/subpageprofile.dart';
import 'package:connettion/ui/subpagetimkiem.dart';
import 'package:connettion/ui/subpagetintuc.dart';
...
class PageMain extends StatelessWidget {
  ...
  PageMain({super.key});
  final List<String> menuTitles = ["Tin tức", "Profile", "Điểm danh", "Tìm kiếm", "Danh sách lớp", "Danh sách học phần"];
  ...
  Widget build(BuildContext context) {
    ...
    else if (viewmodel.activemenu == SubPageDanhsachlop.idpage){
      body = SubPageDanhsachlop();
    }
    else if (viewmodel.activemenu == SubPageDanhsachhocphan.idpage){
      body = SubPageDanhsachhocphan();
    }
          ...
          Consumer<MenuBarViewModel>(
            builder:(context, menuBarViewModel, child) {
              return GestureDetector(
                ...
                child: Container(
                  color: const Color.fromARGB(255, 165, 227, 236),
                  child: Center(
                    child: body,
                  ),
                ),
    ...
    return Column(
      children: [
        SizedBox(
          ...
          child: Center(
            child: AvatarGlow(
              ...
              glowColor: Color.fromARGB(255, 17, 36, 18),//màu bóng
              child: ClipRRect(
                borderRadius: BorderRadius.circular(size.height),
                child: SizedBox(
                  height: size.height * 0.16,
                  width:  size.height * 0.16,
                  child: Image(image: AssetImage('assets/images/giang.jpg'),
                  fit: BoxFit.cover,)),
              ),
            ),
          )
        ...
        Container(
          height: 2,
          width: size.width * 0.65,
          color: Colors.pink[400],// màu đường kẻ ngang thanh menu
        ),
        SizedBox(height: 15,),
        SizedBox(
...
class DrawerCustomPaint extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Paint paint = Paint() 
      ..color = Color.fromARGB(255, 94, 179, 192) //màu khung menu
      ..style = PaintingStyle.fill;
    Path path = Path();
    path.moveTo(0, 0);
    path.lineTo(size.width, 0);
    //path.lineTo(size.width, size.height);
    path.quadraticBezierTo(generatePointX(size.width), offset.dy, size.width, size.height);
